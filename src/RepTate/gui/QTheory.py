...
from RepTate.gui.viewmodels.fit_viewmodel import FitViewModel
from RepTate.gui.viewmodels.posterior_viewmodel import PosteriorViewModel
from RepTate.gui.widgets.fit_plot import FitPlot
from RepTate.gui.widgets.posterior_plot import PosteriorPlot
from RepTate.gui.widgets.diagnostics_panel import DiagnosticsPanel
from RepTate.core.Parameter import OptType, ParameterType


def _student_t_cdf(x: float, dof: int) -> jnp.ndarray:
    v = jnp.asarray(dof, dtype=jnp.float64)
    x_arr = jnp.asarray(x, dtype=jnp.float64)
    t2 = x_arr * x_arr
    ibeta = jsp_special.betainc(v / 2.0, 0.5, v / (v + t2))
    return jnp.where(x_arr >= 0, 1.0 - 0.5 * ibeta, 0.5 * ibeta)


def _student_t_ppf(p: float, dof: int) -> float:
    p_arr = jnp.asarray(p, dtype=jnp.float64)
    x0 = jnp.array([jsp_stats.norm.ppf(p_arr)])
    result = root(lambda x: _student_t_cdf(x[0], dof) - p_arr, x0, bounds=(-100.0, 100.0))
...
class EditThParametersDialog(QDialog):
    ...
    def __init__(self, parent: QWidget, p_name: str) -> None:
...
class QTheory(QWidget, Ui_TheoryTab):
    ...
    def __init__(
        self,
        name: str = "QTheory",
        parent_dataset: object | None = None,
        axarr: Sequence[Any] | None = None,
    ) -> None:
        """
        **Constructor**
        ...
        self.fit_viewmodel: FitViewModel | None = None
        self.posterior_viewmodel: PosteriorViewModel | None = None
    ...
    def do_fit(self, line):
        ...
        tval = _student_t_ppf(1.0 - alpha / 2.0, dof)
