...
def _root_solve(fun, a, b, args=()):
    x0 = jnp.array([(a + b) / 2.0])
    result = optimize.root(lambda x: fun(x[0], *args), x0, bounds=(a, b))
...
class TheoryDSMLinear(QTheory):
    ...
    * **Parameters**"""
    ...
    def __init__(self, name="", parent_dataset=None, axarr=None):
        ...
        solNc = _root_solve(self.solveNc, a=1, b=1000, args=(Gx, Mw, rho0, R, T))
        if solNc > 0:
            self.Nc = solNc
            self.Mc = Mw / self.Nc
