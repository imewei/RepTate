    ...
    * **Parameters**
    ...
    def calculate(self, f=None):
        ...
        * **II. NUMERICAL SETTINGS:**
        ...
        f1 = jinterp1d(jnp.asarray(wp), jnp.asarray(G1G2[:, 1]), kind="cubic")
        f2 = jinterp1d(jnp.asarray(wp), jnp.asarray(G1G2[:, 2]), kind="cubic")
        tt.data[:, 0] = ft.data[:, 0]
        tt.data[:, 1] = np.asarray(f1(jnp.asarray(ft.data[:, 0])))
        tt.data[:, 2] = np.asarray(f2(jnp.asarray(ft.data[:, 0])))
