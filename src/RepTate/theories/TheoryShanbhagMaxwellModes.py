...
def __cumtrapz(y, x):
    dx = x[1:] - x[:-1]
    area = (y[1:] + y[:-1]) * 0.5 * dx
    return jnp.concatenate([jnp.array([0.0]), jnp.cumsum(area)])


def _quad_trapezoid(fn, a, b, *, steps=4096):
    x = jnp.linspace(a, b, steps)
    y = fn(x)
...
class TheoryShanbhagMaxwellModesFrequency(QTheory):
    ...
    * **Parameters**
    ...
    def oldLamC(self, lam, rho, eta):
        ...
        #
        rhoF = jinterp1d(lam, rho, bounds_error=False, fill_value=(rho[0], rho[-1]))
    ...
    def GridDensity(self, x, px, N):
        ...
        fint = jinterp1d(x, px, "cubic")  # smoothen using cubic splines
        pint = fint(xi)  # interpolation
        ci = _cumtrapz(pint, xi, initial=0)
        pint = pint / ci[npts - 1]
    ...
    def costFcn_magic(self, par, g, tau, imode):
        ...
        return _quad_trapezoid(
            lambda x: self.normKern_magic(x, gn, taun, g1, tau1, g2, tau2),
            wmin,
            wmax,
        )
    ...
    def ShanBhagMaxwellModesFrequency(self, f=None):
        ...
        Gp = Gp[indices]
        Gpp = Gpp[indices]
        fp = jinterp1d(w, Gp, fill_value="extrapolate")
        fpp = jinterp1d(w, Gpp, fill_value="extrapolate")
...
class TheoryShanbhagMaxwellModesTime(QTheory):
    ...
    * **Parameters**
    ...
    def MaxwellModesTime(self, f=None):
        ...
        Gexp = Gexp[indices]
        f = jinterp1d(t, Gexp, fill_value="extrapolate")
