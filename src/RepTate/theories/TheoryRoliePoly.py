...
class TheoryRoliePoly(QTheory):
    ...
    def RoliePoly(self, f=None):
        ...
        for i in range(nmodes):
            ...
            if i < nstretch:
                try:
                    sig = rk4_integrate(pde_stretch, sigma0, self.t, p)
                except EndComputationRequested:
                    break
            else:
                try:
                    sig = rk4_integrate(pde_nostretch, sigma0, self.t, p)
                except EndComputationRequested:
                    break
    ...
    def RoliePolyLAOS(self, f=None):
        ...
        for i in range(nmodes):
            ...
            if i < nstretch:
                try:
                    sig = rk4_integrate(pde_stretchLAOS, sigma0, t, p)
                except EndComputationRequested:
                    break
            else:
                try:
                    sig = rk4_integrate(pde_nostretchLAOS, sigma0, t, p)
                except EndComputationRequested:
                    break
