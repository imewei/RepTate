    ...
    * **Parameters**
    ...
    def drag_mode(self, dx, dy):
        ...
        Args:
            ...
            dy (numpy.ndarray): Array of y-coordinates (compliances) for the dragged modes.
        """
    ...
    def get_modes(self):
        """Get the values of Retardation Modes from this theory.

        Returns:
            tuple: A tuple containing (tau, J, success) where tau is the relaxation time array,
                J is the compliance array, and success is a boolean indicating successful retrieval.
        """
        nmodes = self.parameters["nmodes"].value
        tau = np.logspace(
            self.parameters["logtmin"].value, self.parameters["logtmax"].value, nmodes
    ...
    def RetardationModesTime(self, f=None):
        """Calculate the theory prediction for time-dependent Retardation modes.

        Args:
            f (DataFile): The file object containing experimental creep data to fit.
        """
        ft = f.data_table
