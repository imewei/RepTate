...
def findDfStar(params):
    """Find maximum activation barrier for strand nucleation across all NT values.

    Searches for the peak nucleation barrier ΔF* by computing the difference between
    polydisperse and quiescent free energies across a range of total monomer numbers NT.
    The barrier height determines the kinetics of strand formation: higher barriers
    indicate slower nucleation. The function includes the true quiescent landscape
    ...
    Returns:
        Maximum nucleation barrier height max(F_true_qui + F_fluc - F_fluc_qui)
        across all evaluated NT values. Higher values indicate stronger suppression
        of strand nucleation.
    """
    # Extract params
