...
def _student_t_cdf(x, dof):
    v = jnp.asarray(dof, dtype=jnp.float64)
    x = jnp.asarray(x, dtype=jnp.float64)
    t2 = x * x
    ibeta = jsp_special.betainc(v / 2.0, 0.5, v / (v + t2))
    return jnp.where(x >= 0, 1.0 - 0.5 * ibeta, 0.5 * ibeta)


def _student_t_ppf(p, dof):
    p = jnp.asarray(p, dtype=jnp.float64)
    x0 = jnp.array([jsp_stats.norm.ppf(p)])
    result = optimize.root(
        lambda x: _student_t_cdf(x[0], dof) - p, x0, bounds=(-100.0, 100.0)
    )
