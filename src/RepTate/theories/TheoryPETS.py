...
class TheoryPETS(QTheory):
    ...
    def __init__(self, name="", parent_dataset=None, axarr=None):
        ...
        self.has_modes = True
        EPSILON = float(jnp.finfo(jnp.float64).eps)
    ...
    def sigmadot_shear(self, vec, t, p):
        ...
        trQA = QAxx + 2 * QAyy
        trQD = QDxx + 2 * QDyy
        la = jnp.sqrt((lm2 * trQA) / (3 * lm2 - 3 + trQA))
        ld = jnp.sqrt((lm2 * trQD) / (3 * lm2 - 3 + trQD))
        ...
        r_asfree = jnp.minimum(r_asfree, self.RD_MAX)
    ...
    def PETS(self, f=None):
        ...
        p = jnp.array(
            [Z, r_a, lmax, tauD, tauS, tau_as, tau_free, beta, delta, flow_rate],
            dtype=jnp.float64,
        )
        try:
            vec_0_arr = jnp.array(vec_0, dtype=jnp.float64)
            res_vec = odeint(pde, vec_0_arr, t, p, rtol=relerr, atol=abserr)
        ...
        if self.flow_mode == FlowMode.shear:
            ...
            f = np.array(res_vec[1:, 0])
            QAxx = np.array(res_vec[1:, 2])
            QAyy = np.array(res_vec[1:, 3])
            QDxx = np.array(res_vec[1:, 4])
            QDyy = np.array(res_vec[1:, 5])
            #  build stress array
