# RepTate Package Makefile
# ========================
# Development Tools for Rheology of Entangled Polymers
...
.PHONY: help install install-dev install-gui \
        ...
        format lint type-check check quick docs build info version run

# Configuration
PYTHON := python
PYTEST := pytest
PACKAGE_NAME := RepTate
SRC_DIR := RepTate
TEST_DIR := tests
DOCS_DIR := docs
VENV := .venv
...
# ===================
help:
	@echo "$(BOLD)$(BLUE)RepTate Development Commands$(RESET)"
	@echo ""
	...
	@echo "  $(CYAN)run$(RESET)              Launch RepTate GUI application"
	@echo ""
...
install-dev: install
	...
	@$(INSTALL_CMD) pytest pytest-cov black ruff mypy
...
# ===================
run:
	@echo "$(BOLD)$(BLUE)Launching RepTate...$(RESET)"
	$(RUN_CMD) $(PYTHON) -m RepTate

# ===================
# GPU Installation targets
# ===================
...
test-fast:
	...
	$(RUN_CMD) $(PYTEST) $(TEST_DIR) -m "not slow"

test-coverage:
	@echo "$(BOLD)$(BLUE)Running tests with coverage report...$(RESET)"
	$(RUN_CMD) $(PYTEST) $(TEST_DIR) --cov=$(SRC_DIR) --cov-report=term-missing --cov-report=html --cov-report=xml
...
lint:
	...
	$(RUN_CMD) ruff check $(SRC_DIR) $(TEST_DIR)
...
clean-test:
	...
	rm -rf .coverage
	rm -rf coverage.xml
	rm -rf $(SRC_DIR)/tests/output 2>/dev/null || true
...
version:
	@$(PYTHON) -c "from $(PACKAGE_NAME) import __version__; print(__version__)" 2>/dev/null || \
		$(PYTHON) -c "from $(PACKAGE_NAME)._version import get_versions; print(get_versions()['version'])" 2>/dev/null || \
